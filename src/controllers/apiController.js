...
exports.delete_dept = function(req, res) {
  Dept.remove({ dept_id: req.body.dept_id, company: 'arp6333' }, function(err, item) {
    ...
    res.json({ 'success': "Department " + req.body.dept_id + " from arp6333 deleted." });
  });
};
...
exports.update_tc = function(req, res) {
  ...
  Emp.find({ company: 'arp6333', emp_no: req.body.emp_no }, function(err, item) {
    if(err) {
      res.json({ 'error': 'Given emp_no does not exist in our company.' });
    }
...
exports.delete_tc = function(req, res) {
  ...
  res.json({ 'success': "Timecard " + req.body.timecard_id + " deleted." });
  });
};